...
def update_project(
    ...
    # Return format ProjectListItem biar frontend gampang update UI
    total = db.query(ImageRecord).filter(ImageRecord.project_id == project.id).count()
    done = db.query(ImageRecord).filter(ImageRecord.project_id == project.id, ImageRecord.status == "done").count()
...
def delete_project(
    ...
    """Delete a project completely including its files."""
    project = get_project_or_404(db, project_id, current_user.id)

    # 1. Hapus folder fisik project dari storage
